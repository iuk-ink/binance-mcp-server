 ...
 * createIndicatorTools() 将四大类指标合并为一个统一的 ToolDefinition 数组。
 *
 * 指标分类说明：
 * - trendTools: 趋势类（移动平均线、ADX 等）—— 衡量方向
 * - momentumTools: 动量类（RSI、MACD 等）—— 衡量速度
 * - volatilityTools: 波动率类（布林带、ATR 等）—— 衡量振幅
 * - utilityTools: 工具函数（统计、网格等）—— 通用计算
 */
...
export function createIndicatorTools(): ToolDefinition[] {
  return [...trendTools, ...momentumTools, ...volatilityTools, ...utilityTools];
}

// 按类别单独导出，便于按需引用
export { trendTools, momentumTools, volatilityTools, utilityTools };
