...
export const trendTools: ToolDefinition[] = [
  { name: 'indicator_sma', description: '简单移动平均线 (SMA)', schema: SMAInput, handler: async (a: unknown) => { const { values, interval } = a as { values: number[]; interval: number }; try { const i = new SMA(interval); const r: number[] = []; for (const v of values) { const x = i.add(v); if (x !== null) r.push(x); } return ok({ indicator: 'SMA', interval, values: r, last: i.getResult() }); } catch (e) { logError(e as Error); return ok({ error: true, message: (e as Error).message }); } } },
  ...
  { name: 'indicator_wma', description: '加权移动平均线 (WMA)', schema: WMAInput, handler: async (a: unknown) => { const { values, interval } = a as { values: number[]; interval: number }; try { const i = new WMA(interval); const r: number[] = []; for (const v of values) { const x = i.add(v); if (x !== null) r.push(x); } return ok({ indicator: 'WMA', interval, values: r, last: i.getResult() }); } catch (e) { logError(e as Error); return ok({ error: true, message: (e as Error).message }); } } },
  { name: 'indicator_wsma', description: "Wilder's 平滑移动平均线 (WSMA)", schema: WSMAInput, handler: async (a: unknown) => { const { values, interval } = a as { values: number[]; interval: number }; try { const i = new WSMA(interval); const r: number[] = []; for (const v of values) { const x = i.add(v); if (x !== null) r.push(x); } return ok({ indicator: 'WSMA', interval, values: r, last: i.getResult() }); } catch (e) { logError(e as Error); return ok({ error: true, message: (e as Error).message }); } } },
